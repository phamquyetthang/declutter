...
i18n_load() {
  if [ "$DECLUTTER_LC" = vi ]; then
    ...
    T_PARTIAL="✓ (một phần bị từ chối quyền)"
    T_CMD_FAILED="✗ lệnh lỗi — xem log"
    T_CMD_RUNNING="…"
    T_RESULT="Kết quả"
    T_ITEMS_DONE="  %d mục đã xử lý"
    T_ITEMS_DENIED=" (%d mục có lỗi — xem log)"
    ...
    T_PARTIAL="✓ (some paths denied)"
    T_CMD_FAILED="✗ command failed — see the log"
    T_CMD_RUNNING="…"
    T_RESULT="Result"
    T_ITEMS_DONE="  %d items processed"
    T_ITEMS_DENIED=" (%d finished with errors — see the log)"
